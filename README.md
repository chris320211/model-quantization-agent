...
3. **Staged Adapt** acquires the chosen repository, creates a repository-derived install/entrypoint plan, builds its venv, and learns the target model's exact architecture. When upstream does not document that model family, it creates a separate reviewable overlay port without modifying the canonical checkout. It then authors a script and validates syntax, imports, exact model/output values, tune locks, and the overlay contract (up to 3 retries).
...
The Research agent uses `ChatOpenAI.with_structured_output(ResearchReport)` over the OpenAI Responses API — not a ReAct loop — and grounds purely on the structured catalog (no RAG). All requests set `store=False`. The Adapt agent is a staged set of `create_react_agent` calls that clones the method repo, introspects the target model's architecture, reads the method's paper, optionally creates a separate port overlay, and writes a validated script.

Model policy is stage-specific:

- research, planning, authoring: `gpt-5.6-terra` with medium reasoning;
- overlay porting and runtime repair: `gpt-5.6-sol` with high reasoning;
- tuning and hyperparameter extraction: `gpt-5.6-terra` with low reasoning.

`QUANT_AGENT_MODEL` overrides every stage. Prefer a narrower override such as `QUANT_AGENT_PORT_MODEL` or `QUANT_AGENT_FIX_MODEL`; reasoning can be changed with `QUANT_AGENT_<STAGE>_REASONING_EFFORT` (`low`, `medium`, or `high`). Valid stage names are `RESEARCH`, `PLAN`, `AUTHOR`, `PORT`, `FIX`, `TUNE`, and `HYPERPARAM`.
...
- `src/quant_agent/llm.py` — OpenAI Responses API construction and per-stage model policy.
