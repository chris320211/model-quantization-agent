...
- The value never lands in this Codex conversation.
...
```
source /home/ubuntu/model-quantization-agent/.Codex/skills/_shared/load_env.sh
...
Anthropic(api_key=os.environ['ANTHROPIC_API_KEY']).messages.create(
    model=os.environ.get('QUANT_AGENT_MODEL', 'Codex-sonnet-4-6'),
...
| `QUANT_AGENT_MODEL` | optional | Python CLI | Model id override; defaults to `Codex-sonnet-4-6`. |
