...
`jobs/<job_id>/benchmark.json`, port `ranked`, authored `diagnose_fix`
overlays, wrapper `# QUANT_AGENT_OVERLAY_DIR=`, `overlay.patch` sha256,
`tried_overlays`, and `retry_gpu_jobs_*`. Return its JSON.
...
| `eval_runtime_flags_missing` | `eval_runtime_flags_missing` | Packed/fused weights loaded without restoring `_eval_mode` / `use_diag=False` (not in state_dict), so transforms apply twice and PPL explodes. |
...
2. **`author_fix`** — no remaining overlay keeps runtime, and retry budget
   ...
   `.linear.weight` into vanilla HF. Read `issue_codes` from diagnose JSON;
   do not retune from raw PPL.
