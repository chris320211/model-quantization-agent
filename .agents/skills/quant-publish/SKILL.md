...
description: >-
  ...
  comparison board (`compare/` ranks methods on the same model × GPU).
...
Upload (needs `HF_TOKEN` / `HUGGINGFACE_HUB_TOKEN` already in the parent
environment; never paste the value into chat):
